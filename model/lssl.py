...
class LegTTransition(AdaptiveTransition):
    ...
    def forward_mult(self, u, delta, transpose=False):
        if transpose:
            return legt_gbt_forward_t(delta, u, transpose=True)
        else:
            return legt_gbt_forward(delta, u)

    def inverse_mult(self, u, delta, transpose=False):
        if transpose:
            return legt_gbt_backward_t(-delta, u, transpose=True)
        else:
            return legt_gbt_backward(-delta, u)
